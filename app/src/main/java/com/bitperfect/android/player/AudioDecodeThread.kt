 ...
 *
 * STATUS: not currently on any playback path, and deliberately kept.
 *
 * PlaybackController drives AudioTrackPlaybackSink, which decodes with the
 * native WAV/FLAC decoders and renders through Android AudioTrack. That is the
 * on-device path. This class is the other half: it is the only thing that feeds
 * the native engine's ring buffer, which is what the USB isochronous transport
 * consumes, so it is what a USB DAC output backend would be built on.
 *
 * Note that it decodes via MediaExtractor/MediaCodec rather than the native
 * decoders, so a USB backend built on it should move to the decoder-session API
 * on NativeAudioEngine to keep one decode path for both outputs.
 *
